...
export function getExpectedCounts(wkspUri: vscode.Uri, config: Configuration): WkspParseCounts {
  ...
  return {
    // 5 nodes = path-group (added per fileParser.ts:366 when entry.source is
    // 'config-file' OR there are multiple featuresUris) + feature file + 3 scenarios.
...
export const getExpectedResults = (wkspUri: vscode.Uri, config: Configuration): TestResult[] => {
  ...
  const expectedResults: TestResult[] = [
    new TestResult({
      ...
      scenario_featureName: 'Malformed Config Fallback',
      ...
      test_id: '.../malformed-config/{{featurePath}}/discovery.feature/run a successful test',
      test_label: 'run a successful test',
      test_parent: '.../malformed-config/{{featurePath}}/discovery.feature',
      test_uri: '.../malformed-config/{{featurePath}}/discovery.feature'
    }),
    ...
    new TestResult({
      ...
      test_description: undefined,
      test_error: undefined,
      test_id: '.../malformed-config/{{featurePath}}/discovery.feature/run a failing test',
    ...
    new TestResult({
      ...
      test_description: undefined,
      test_error: undefined,
      test_id: '.../malformed-config/{{featurePath}}/discovery.feature/run a skipped test',
