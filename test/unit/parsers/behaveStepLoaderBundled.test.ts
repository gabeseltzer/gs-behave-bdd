// Unit tests for behaveLoader - bundled libs argument passing
...
suite('behaveLoader bundled libs', () => {
  ...
  test('passes --bundled-libs argument when bundledLibsPath is provided', async () => {
    ...
    await loadFromBehave('python', '/project', ['/project/steps'], bundledPath);
  ...
  test('does not pass --bundled-libs when bundledLibsPath is undefined', async () => {
    ...
    await loadFromBehave('python', '/project', ['/project/steps']);
  ...
  test('falls back to bundled behave when environment behave is not installed', async () => {
    ...
    spawnStub = sinon.stub(childProcess, 'spawn').callsFake(() => {
      ...
      setImmediate(() => {
        if (callCount === 1) {
          ...
          proc.stdout.emit('data', '{"steps":[],"fixtures":[]}');
          proc.emit('close', 0);
        }
      });
    ...
    const result = await loadFromBehave('python', '/project', ['/project/steps']);

    assert.strictEqual(callCount, 2, 'spawn should be called twice (original + fallback)');
    assert.deepStrictEqual(result.steps, [], 'should return empty steps from fallback');
    assert.deepStrictEqual(result.fixtures, [], 'should return empty fixtures from fallback');
  ...
  test('does not fall back when bundledLibsPath was already provided', async () => {
    ...
    await assert.rejects(
      async () => await loadFromBehave('python', '/project', ['/project/steps'], getBundledBehavePath()),
  ...
  test('does not fall back for non-behave import errors', async () => {
    ...
    await assert.rejects(
      async () => await loadFromBehave('python', '/project', ['/project/steps']),
      /import.*error/i,
