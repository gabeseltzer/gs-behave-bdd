...
| 2026-05-18 | fix-fatal-settings-cascade | `b7b8f90`, `bb7d610`, `f88c0cc` | When a workspace had bad `projectPath`/`featuresPaths` settings, `WorkspaceSettings` ctor's WkspError was caught by the `config.workspaceSettings` getter (which surfaces one notification), but two downstream call sites then blind-dereferenced the now-undefined entry and threw cascading "Cannot read properties of undefined" errors — user saw three error notifications for one underlying misconfig. Added early-return guards: `startWatchingWorkspace` returns `[]`; `parseFilesForWorkspace` marks parse-state flags finished, disposes the just-allocated cancel token, and returns undefined. Both guards `diagLog` only — no second notification. +7 regression tests (3 watcher + 4 parser); 884 unit tests passing; eslint clean. |
| 2026-05-18 | clearer-fatal-settings-error | `849e94b`, `3d653b5`, `cbec99e`, `eac494f` | Follow-up to fix-fatal-settings-cascade. Three remaining noise sources after the cascade guards: (1) `workspaceSettings` getter re-constructed `WorkspaceSettings` on every access for failed uris, so settings dumps + FATAL throws repeated 4x; (2) `extension.ts:364` logged WkspError as `Phase 21 migration consent flow error` jargon; (3) `No "steps" folder found` warn fired alongside FATAL as a derivative symptom; plus a verbose 3-line toast with no action buttons. Fixes: (A) `_failedSettingsWorkspaces` upgraded to `Map<string, Error>`, getter short-circuits on cache hit; `reloadSettings` clears entry. (B) Steps warn gated on `_fatalErrors.length === 0`. (C) Toast reformatted to single line `Behave BDD: project path "<value>" not found in workspace "<name>". Tests cannot load.` with `[Open Settings]` / `[Show Details]` / `[Reload Window]` buttons via new `WkspError.actions` + `logger.showError` button-rendering plumbing. `__showOutput` sentinel routes Show Details to `logger.show()` (avoids registering a one-purpose public command). (D) Output-channel `_fatalErrors` quote both setting value and resolved path. Phase 21 catch in `extension.ts:364` now `diagLog`s WkspError instances (already surfaced by getter). +14 regression tests (5 configuration + 8 fatalToast + 1 updated); 898 unit tests passing; eslint clean. |
