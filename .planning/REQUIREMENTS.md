# Milestone 1.2.0: Multi-Path & Monorepo-Aware Discovery — Requirements
...
- [ ] **SD-01**: New `src/discovery/configScanner.ts` module implements BFS-bounded subdir scan (`scanForBehaveConfig(wkspUri, maxDepth): ScanResult`). Respects `DEFAULT_EXCLUDE_DIRS` (adds `dist`, `out`, `build`, `coverage`); protects against symlink cycles via visited-realpath set; circuit-breaks at configurable `maxEntriesScanned`; iteration order is depth-outer/filename-inner (preserves 1.0.0 D-06 malformed-config priority).
...
- [ ] **SD-04**: `configWatcher.ts` glob upgrades from `{configs}` to `**/{configs}` with two-tier watcher strategy (Tier 1: narrow watcher at discovered config's directory for speed; Tier 2: recursive `**/` only when no primary config discovered). Preserves 1.1.0 brace-expansion fix (#164925) and 500ms debounce (#72831).
...
- [ ] **INT-03**: `projectPath` manual override still wins over subdir scan — 1.0.0 priority chain (manual > config > convention) preserved and explicitly re-tested.
- [ ] **INT-04**: `integrationTestRun` bypass — all new re-discovery paths (scanner-triggered rebuild, multi-path watcher rebuild) call the cache + parser directly, not through `configurationChangedHandler`, mirroring 1.1.0 Pitfall 14 handling.
...
- [ ] **TEST-14**: Dedicated fixtures — `example-projects/multi-path/` (single config with multi-value `paths=`) and `example-projects/monorepo-scan/` (nested `app-a/behave.ini` + `app-b/behave.ini` + seeded `node_modules/` for perf assertion). Isolated per 1.1.0 D-05 pattern.
- [ ] **TEST-15**: 3× flakiness gate on Windows CI (1.1.0 precedent) for the new integration suite before milestone close.

---

## Future Requirements

(None currently deferred at the 1.2.0 level — full scope elected. Any MP-04 or MP-05 complications during execution may be bumped to a 1.2.x patch release.)
...
The following features would require `WorkspaceSettings` to become per-project rather than per-workspace-folder. They are explicitly out of scope for 1.2.0 and must NOT ship:
...
- **Hard-blocking run guard on alsoFoundConfigs ambiguity** — anti-feature; user must always be able to proceed (1.1.0 precedent).
...
- **Prior milestones:** `.planning/milestones/1.0.0-REQUIREMENTS.md`, `.planning/milestones/1.1.0-REQUIREMENTS.md`

*REQ-ID format: `[CATEGORY]-[NUMBER]`. Continuing from existing prefixes (MP-, SD-, INT-, TEST-); TEST- continues from 1.1.0 TEST-09 → 1.2.0 starts at TEST-10.*
