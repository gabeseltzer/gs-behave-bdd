...
5. Open a command line window, go to your source folder, and clone the extension source code, example:
    - `cd <mysourcedir>`
    - `git clone https://github.com/gabeseltzer/gs-behave-bdd.git`
6. Change to the cloned directory, and install required node packages:
    - `cd <mysourcedir>/gs-behave-bdd`
    - `npm install`
...
9. Install [uv](https://docs.astral.sh/uv/getting-started/installation/) and set up the Python dev tooling virtual environment (for mypy and ruff):
    ...
    - Change back to the cloned directory: `cd <mysourcedir>/gs-behave-bdd`
    - `uv sync`
...
3. Open vscode, and open the gs-behave-bdd source folder (close any other folders you have open).
...
- You probably want to enable `gs-behave-bdd.xRay` in your settings.json so you get diagnostic logs in debug console in the source environment.
...
- Diagnostic logs are controlled via the extension setting `gs-behave-bdd.xRay` (this is enabled by default in the example projects and for most integration tests).
...
- Note that stack traces will only appear if `gs-behave-bdd.xRay` is enabled.
